...
def signup():
    ...
    if account=="3318" and password=="3318":
...
def count():
    ...
    for volname in volname_list:
       ...
       cursor.execute("SELECT workday FROM volun WHERE volname = %s", (volname,))
       workday1 = int(cursor.fetchone()[0])
       workday11 = workday1 - 1
       cursor.execute("SELECT workday1 FROM volun WHERE volname = %s", (volname,))
       workday2 = int(cursor.fetchone()[0])
       workday22 = workday2 - 1

       today = date.today()

       cursor.execute("SELECT * FROM holiday WHERE holiday <= %s and day = %s", (today, workday1))
       result1 = cursor.fetchall()
       num_records1 = len(result1)
       cursor.execute("SELECT * FROM holiday WHERE holiday <= %s and day = %s", (today, workday2))
       result2 = cursor.fetchall()
       num_records2 = len(result2)

       year = date.today().year
       start_month = 1
       end_month = date.today().month
       hours = count_weekday(year, start_month, end_month, [workday11]) - num_records1
       hours1 = count_weekday(year, start_month, end_month, [workday22]) - num_records2
       hourst = ((hours + hours1) * 3)+hou

       cursor.execute("UPDATE hours SET hourst = %s  WHERE volname = %s", (hourst, volname,))
       cursor.execute("UPDATE hours SET hours = %s  WHERE volname = %s", (hours, volname,))
       cursor.execute("UPDATE hours SET hours1 = %s  WHERE volname = %s", (hours1, volname,))
       con.commit()
    ...
    SELECT sub.volname,
        ...
        sub.遲到早退次數,
        sub.支援次數,
        hours.hourst AS 時數欄位
    FROM (
        SELECT volname,
            SUM(CASE WHEN reason = '請假有代班' THEN 1 ELSE 0 END) AS 請假有代班次數,
            SUM(CASE WHEN reason = '請假未代班' THEN 1 ELSE 0 END) AS 請假未代班次數,
            SUM(CASE WHEN reason = '未請假' THEN 1 ELSE 0 END) AS 未請假次數,
            SUM(CASE WHEN reason = '代班' THEN 1 ELSE 0 END) AS 代班次數,
            SUM(CASE WHEN reason = '換班' THEN 1 ELSE 0 END) AS 換班次數,
            SUM(CASE WHEN reason = '被換班' THEN 1 ELSE 0 END) AS 被換班次數,
            SUM(CASE WHEN reason = '遲到早退' THEN 1 ELSE 0 END) AS 遲到早退次數,
            SUM(CASE WHEN reason = '支援' THEN 1 ELSE 0 END) AS 支援次數
        FROM reason
        GROUP BY volname
    ) AS sub
    JOIN hours ON sub.volname = hours.volname
    '''
